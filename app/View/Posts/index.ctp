...
<div class="posts index">
	...
	<div class="Lmargin50">	
		...
		<tr>
				<th><?php echo $this->Paginator->sort('menu_id','Մենյու',array('class'=>'C1'));?></th>
		...
		<tr class= "<?php  if (($i++)%2 == 1){ echo 'bg';}?>">
			<td>
				<?php echo $this->Html->link($post['Menu']['name'], array('controller' => 'menus', 'action' => 'view', $post['Menu']['id']),array('class'=>'C1')); ?>
			</td>
			<td><?php echo h($post['Post']['name']); ?>&nbsp;</td>
			<td><?php echo h($stick[$post['Post']['stick']]); ?>&nbsp;</td>
			<td><?php echo h($post['Post']['created']); ?>&nbsp;</td>
			<td class="actions">
				<?php echo $this->Html->link(__('Դիտել'), array('action' => 'view', $post['Post']['id']),array('class'=>'C1')); ?>
				<?php echo $this->Html->link(__('Փոփոխել'), array('action' => 'edit', $post['Post']['id']),array('class'=>'C1')); ?>
				<?php echo $this->Form->postLink(__('Ջնջել'), array('action' => 'delete', $post['Post']['id']),array('class'=>'C1'), __('Դուք համոզված եք դուք ցանկանւմ եք ջնջել %s Նյութը', $post['Post']['name'])); ?>
			</td>
		</tr>
	<?php endforeach; ?>
		</table>        
